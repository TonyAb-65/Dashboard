...
# -------- Persistent cache across reruns (keyed by image hash, not SKU) --------
@st.cache_resource
...
# ============== HELPERS ==============
STOP = {
    "the","and","for","with","of","to","in","on","by","a","an","&","-",
...
def safe_section(label, fn):
    try:
        st.markdown(
            f"<span style='color:#94a3b8;font-size:12px'>entering {label}</span>",
            unsafe_allow_html=True,
        )
        return fn()
...
with st.sidebar:
    ...
    CONTEXT_HINT = st.text_input("Optional translation context",
                                 value="E-commerce product titles for a marketplace.")

    st.markdown("---")
    DEBUG = st.checkbox("🪲 Debug mode (log payloads)", value=False)
    section = st.radio(
        "Navigate",
        ["📊 Overview","🔎 Filter","🖼️ Titles & Translate","🧩 Grouping",
         "📑 Sheet","⬇️ Downloads","⚙️ Settings"],
        index=0
    )
...
with c1:
    product_file = st.file_uploader(
        "Product List (.xlsx/.csv, includes 'thumbnail')",
        type=["xlsx","xls","csv"], key="u1"
    )
with c2:
    mapping_file = st.file_uploader(
        "Category Mapping (.xlsx/.csv)",
...
loaded_ok = (
    ...
    validate_columns(map_df, ["category_id","sub_category_id","sub_category_id NO",
...
def mapped_mask_fn(df: pd.DataFrame) -> pd.Series:
    if df is None or df.empty or "sub_category_id" not in df or "sub_sub_category_id" not in df:
        return pd.Series([False] * (0 if df is None else len(df)),
                         index=([] if df is None else df.index))
    return df["sub_category_id"].fillna("").astype(str).str.strip().ne("") & \
           df["sub_sub_category_id"].fillna("").astype(str).str.strip().ne("")
...
def build_lookups(map_df: pd.DataFrame):
    ...
    ssub_no = {(r["category_id"], r["sub_category_id"], r["sub_sub_category_id"]): r["sub_sub_category_id NO"]
...
# enforce string dtype for title columns to avoid pandas dtype warnings
...
lookups = build_lookups(map_df) if map_df is not None else {
    "main_names": [], "main_to_subnames": {}, "pair_to_subsubnames": {},
    "sub_name_to_no_by_main": {}, "ssub_name_to_no_by_main_sub": {}
}
...
# ===== Titles & Translate (strict line-by-line: image → EN → AR; ignore SKU) =====
...
def _fallback_simple_title_from_dataurl(data_url: str, max_chars: int) -> str:
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role":"user","content":[
                {"type":"text","text":"Write ONE clean English product title ≤70 chars. Order: Brand, Product type, Variant/Flavor/Color/Scent/Material, Size/Count. No marketing."},
...
def openai_title_from_url(img_url: str, max_chars: int) -> str:
    ...
    try:
        ...
        return _fallback_simple_title_from_dataurl(data_url, max_chars)
    if not ((data.get("object_type") or "").strip() or (data.get("product") or "").strip()):
        return _fallback_simple_title_from_dataurl(data_url, max_chars)
    title = assemble_title_from_fields(data)
...
def translate_en_titles(titles_en: pd.Series, engine: str, batch_size: int,
                        use_glossary=False, glossary_map: Optional[Dict[str,str]] = None,
                        context_hint: str = "") -> pd.Series:
    ...
    if engine == "DeepL" and deepl_active:
        return pd.Series(deepl_batch_en2ar(texts, context_hint), index=titles_en.index, dtype="string")
    if engine == "OpenAI":
        out=[]
        for s in range(0, len(texts), max(1, batch_size)):
            out.extend(openai_translate_batch_en2ar(texts[s:s+batch_size]))
            ui_sleep(0.1)
        return pd.Series(out, index=titles_en.index, dtype="string")
    return pd.Series(texts, index=titles_en.index, dtype="string")
...
def sec_titles():
    ...
    if st.button("Preview 24 images (no processing)", key="btn_preview_imgs"):
        ...
        if "thumbnail_dataurl" in view.columns:
            ...
            for j, (i, row) in enumerate(view.iterrows()):
                ...
                with cols[j % 6]:
                    if src.startswith("data:") or is_valid_url(src):
                        st.image(src, caption=f"Row {i}", width="stretch")
                    else:
                        st.caption("Bad image")
        else:
    ...
    def _image_cache_key(i: int) -> str:
        ...
        basis = data_col[:256] if data_col.startswith("data:") else clean_url_for_vision(raw_thumb)
        return hashlib.sha1(basis.encode()).hexdigest()[:12] if basis else f"row-{i}"
        ...
        for s in range(0, len(idx), fetch_batch):
            ...
            for i in chunk:
                ...
                if not (data_col.startswith("data:") or is_valid_url(norm_url)):
                    st.session_state.audit_rows.append({"row": int(i), "phase": "EN title", "reason": "no_image_source", "url": raw_thumb})
                    failed += 1; continue
            ...
            if en_out:
                idxs = list(en_out.keys())
                vals = [en_out[j] for j in idxs]
    ...
    def run_trans(idx, trans_batch, engine, force_over) -> Tuple[int,int]:
        ...
        for i in idx:
            ...
            if not en:
                st.session_state.audit_rows.append({"row": int(i), "phase": "AR translate", "reason": "missing EN",
                                                    "url": str(work.at[i, "thumbnail"])})
                continue
            cache_key = _image_cache_key(i)
...
def sec_grouping():
    ...
    with left:
        ...
        new_kws = st.text_area("Add keywords (one per line)",
                               placeholder="soap\nshampoo\ndishwashing\nlemon",
                               key="kw_add")
        ...
        if st.button("🗑️ Remove selected", key="kw_rm_btn"):
            s = set(rm)
            st.session_state.keyword_library = [k for k in st.session_state.keyword_library if k not in s]
    ...
    with right:
        ...
        def hits(df, term):
            tl = term.lower()
            m = df["name"].astype(str).str.lower().str.contains(tl, na=False) | \
                df["name_ar"].astype(str).str.lower().str.contains(tl, na=False)
            return int(m.sum())
        opts = []; keymap = {}
